...
class Cpu:
    ...
    def __init__(self, model, cores, ghz, cost, name, cachemb=0, watts=0, family='xeon', minCount=0):
        ...
        model.addConstr(self.countVar - 10000 * self.useVar, GRB.LESS_EQUAL, 0, "distinctCpus")
...
class Memory:
    ...
    def __init__(self, model, gb, memoryType, mhz, cost, ddr='DDR4'):
        ...
        self.countVar = model.addVar(lb=0, vtype=GRB.INTEGER, name=baseVarName)
        self.useVar = model.addVar(vtype=GRB.BINARY, name="use " + baseVarName)
        model.addConstr(self.countVar - 10000 * self.useVar, GRB.LESS_EQUAL, 0, "distinctMemory")
...
class Disk:
    ...
    def __init__(self, model, tb, size, cost, iops, protocol, gbps, partnumber, disktype='HDD'):
        ...
        self.countVar = model.addVar(lb=0, vtype=GRB.INTEGER, name=baseVarName)
        self.useVar = model.addVar(vtype=GRB.BINARY, name="use " + baseVarName)
        model.addConstr(self.countVar - 10000 * self.useVar, GRB.LESS_EQUAL, 0, "distinctDisks")
...
class Network:
    ...
    def __init__(self, model, cost, speed, adapterType, adapterCount, manufacturer):
        ...
        self.countVar = model.addVar(lb=0, vtype=GRB.INTEGER, name=baseVarName)
        self.useVar = model.addVar(vtype=GRB.BINARY, name="use " + baseVarName)
        model.addConstr(self.countVar - 10000 * self.useVar, GRB.LESS_EQUAL, 0, "distinctNetworkCards")
...
class Server:
    ...
    def __init__(self, model, name, url, cost, watts, rackunits, cpuslots, memoryslots, minmemoryslots, disk25slots, disk35slots, nvmeslots, minimum25disks, minimum35disks, osdisks, osdisktype, x16, x8, onboardnetworkports, siomcards, powersupplies, nodes):
        ...
        self.countVar = model.addVar(lb=0, vtype=GRB.INTEGER, name=baseVarName)
        self.useVar = model.addVar(vtype=GRB.BINARY, name="use " + baseVarName)
        model.addConstr(self.countVar - 10000 * self.useVar, GRB.LESS_EQUAL, 0, "distinctServers")
...
def buildVars(name):
    ...
    return m, cpus, memory, disk25, disk35, diskNvme, siom, pciNetwork, servers


def ilp():
    m, cpus, memory, disk25, disk35, diskNvme, siom, pciNetwork, servers = buildVars("ILP")
...
minDiskIOPS = 40000
maxDistinctDisks = 1
fillAllDiskSlots = False
minTotalNetworkSpeedGigabits = 6000
